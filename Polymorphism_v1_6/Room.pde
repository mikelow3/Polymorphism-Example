  ...
  println("I am a Room"); 
   ...
   boolean checkCollision(Shape s1) 
    { 
      ...
      if(s1 instanceof Room) //rect to rect collision
       {
       ...
       else if(s1 instanceof Zombie) //rect to circle collison
       {
          float circleDistanceX = abs(s1.getxPosition() - xRoomPosition - roomWidth/2);
          float circleDistanceY = abs(s1.getyPosition() - yRoomPosition - roomHeight/2);
 
          if (circleDistanceX > (roomWidth/2 + s1.getRadius())) { return false; }
          if (circleDistanceY > (roomHeight/2 + s1.getRadius())) { return false; }
          ...
          return (cornerDistance_sq <= pow(s1.getRadius(),2));
        }
