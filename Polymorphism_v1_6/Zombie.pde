  ...
  println("I am a zombie"); 
    ...
    { 
      ...
      if(s1 instanceof Zombie)
         ...
         return dist(xZombiePosition, yZombiePosition, s1.getxPosition(), s1.getyPosition()) < (zombieRadius)+(s1.getRadius());
       }
       
       else if(s1 instanceof Room)
       {
         //checks for overlap of ellipse/rect objects
          float circleDistanceX = abs(xZombiePosition - s1.getxPosition() - s1.getWidth()/2);
          float circleDistanceY = abs(yZombiePosition - s1.getyPosition() - s1.getHeight()/2);
 
          if (circleDistanceX > (s1.getWidth()/2 + zombieRadius)) { return false; }
          if (circleDistanceY > (s1.getHeight()/2 + zombieRadius)) { return false; }
 
          if (circleDistanceX <= (s1.getWidth()/2)) { return true; }
          if (circleDistanceY <= (s1.getHeight()/2)) { return true; }
 
          float cornerDistance_sq = pow(circleDistanceX - s1.getWidth()/2, 2) +
          pow(circleDistanceY - s1.getHeight()/2, 2);
